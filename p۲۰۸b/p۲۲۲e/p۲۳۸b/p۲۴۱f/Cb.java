package p۲۰۸b.p۲۲۲e.p۲۳۸b.p۲۴۱f;
...
public class Cb extends AbstractCa {
    ...
    public Float mo۱۴۰۷۰a(float t, float b2, float c2, float d2) {
        float t2 = t / (d2 / 2.0f);
        if (t2 < 1.0f) {
            return Float.valueOf((((-c2) / 2.0f) * (((float) Math.sqrt(1.0f - (t2 * t2))) - 1.0f)) + b2);
        }
        float t3 = t2 - 2.0f;
        return Float.valueOf(((c2 / 2.0f) * (((float) Math.sqrt(1.0f - (t3 * t3))) + 1.0f)) + b2);
    }
}
